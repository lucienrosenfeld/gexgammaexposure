...
**Research-round amendment entries (recorded 2026-07, pre-Phase-1).** Three entries, adopted under the governing rule diagnostics > fixed transformations > replacement features > new fitted coefficients > latent models. The prespecified registry is split into predictive configurations (which floor the trial denominator; the seventeen above plus imbalance=I1beta) and non-predictive entries (R-NEFF-01, D-STAGE2-ID-01), which must neither inflate nor dilute it. Two new append-only record types: pre-outcome rejections (a configuration rejected under a prespecified rule before any outcome inspection leaves both the floor and m_unrun; the record shows the frozen rule, the computed statistic, and the no-outcome-inspection attestation) and deferrals (the entry stays in both until resolved).

**R-NEFF-01, effective correlated trial count (methodological correction, not a trial).** For every executed prespecified configuration, the daily out-of-sample net-return series over the common intersection of evaluation dates, eligible-flat days recorded as 0.0, trade-only correlation matrices prohibited. N_eff_run = (Σλ)²/Σλ² over the eigenvalues of the correlation matrix; N_eff = N_eff_run + m_unrun, each prespecified-but-unexecuted configuration counting as one fully independent trial — declining to run a candidate can never lower the denominator. Zero-variance patch: an executed configuration trading zero days over the common intersection is removed from the matrix and counted as one fully independent trial (expected, not hypothetical, under flat-is-admissible). Validity conditions, all required: common intersection ≥ 500 daily observations; observations ≥ 10 × executed configurations; PSD correlation matrix; N_eff on the first and last 80% of common dates within 25% of the full estimate. Any failure ⇒ the plain Harvey–Liu count controls. Both the effective and raw-count results are always computed and logged side by side; the correction may change the headline, never hide the conservative number. Recorded weaknesses, documented not corrected: (a) zero-filling depresses correlations between strategies with differing trade dates, biasing N_eff toward the raw count — conservative, and never to be "corrected" via pairwise-overlap or trade-only correlations; (b) the 80%/80% stability windows share 60% of observations, overstating stability — tolerable only because failure defaults to the conservative estimator; (c) the participation ratio measures effective dimensionality of the return covariance, not the effective trial count of a max-statistic over correlated tests, and understates the latter at moderate correlation — a valid participation ratio is a lower bound on effective trials, not the correct value.

**R-IMB-BETA-01, beta-weighted auction imbalance (conditional predictive trial).** One counted replacement for the cap-weighted aggregation, never an additional regressor: I_β = Σ w_i β̃_i q_i versus baseline I_w = Σ w_i q_i. Frozen beta pipeline: rolling covariance beta of daily close-to-close constituent returns against ES, 252-day window ending t−1, point-in-time membership only, minimum 120 paired observations; shrinkage β_shrunk = 0.5·β̂ + 0.5, then clip to [0.25, 2.5]; below 120 observations β̃ = 1. No same-day return may enter; no alternative lookback, shrinkage, prior, clip, frequency, or fallback may be tested under this entry. Under Spec I₁ the candidate produces [I_β,55, ΔI_β]; the level inherits w_I ≥ 0, the change remains unconstrained. Pre-registration redundancy screen: ρ = Corr[z(I_β), z(I_w)] over all available point-in-time history on the production standardisation; |ρ| > 0.90 ⇒ rejected before predictive evaluation, recorded in Appendix C, excluded from the trial denominator (threshold frozen before computation; no outcomes inspected; no substitute beta construction in the same round). Screen sample floor: valid only on ≥ 250 point-in-time trading days of constituent imbalance history; below the floor the screen and the trial are deferred, not decided — the configuration remains in m_unrun until the floor is met. The usable-day count under the strictly lagged standardisation is logged alongside the calendar count. A rejection at the screen is a successful result: mega-cap betas cluster near one and dominate cap weights, so redundancy is a live prior, discovered at zero denominator cost.

**D-STAGE2-ID-01, Stage 2 identification diagnostics (infrastructure, not a trial).** Per Stage 2 training fold, on the exact standardised design used in estimation: Gram matrix, condition number κ_f, pairwise correlations, VIFs, coefficient signs and magnitudes with fold-level standard errors (frozen method: stationary block bootstrap on the training fold, identical across folds), adjacent-fold sign changes, and the design-variance share of the smallest eigenmode. Training-fold information only. Alarm: κ_f ≥ 10 and a material adjacent-fold sign change, material meaning |coef| exceeds its fold-level SE on both sides of the flip. Rationale: with three standardised columns κ = 30 requires pairwise ρ ≈ 0.94 while ridge sign instability begins near ρ ≈ 0.8 (κ ≈ 9); and Section 26's honest prior is β₁ ≈ 0, whose sign flips as pure noise — without the materiality floor, the model behaving exactly as predicted would manufacture alarms. Persistent non-identification: alarms on ≥ 30% of comparable adjacent-fold transitions, minimum three. Consequence: nothing automatic — it permits one later, separately registered replacement hypothesis (collapse Stage 2 into one fixed, economically signed composite), specified before viewing predictive results and counted as a new trial. Scope resolution, recorded: the materiality floor also governs the sign-instability *removal* rule when applied to Stage 2 features, since removing a coefficient for being near zero would punish the predicted outcome; the Stage 1 removal rule is unchanged and unfiltered, its constrained coefficients shrinking to exact zeros, which the rule already treats as neutral. This entry is measurement infrastructure for the removal rule, and no code path in it may mutate features, constraints, or model structure.
...
**Amendment window (closing rule of Part VII).** The specification is frozen at the research-round amendment commit. No further amendments are admissible until Decision Gate 1 returns an answer. Any amendment proposed after Phase 1 results exist is inadmissible regardless of content — the merit of a post-results amendment cannot be evaluated separately from the results that prompted it.
...
Research-round amendment constants, all frozen. R-IMB-BETA-01: beta lookback 252 days; minimum 120 paired observations; shrinkage 0.5 toward 1; clip [0.25, 2.5]; fallback 1.0; screen threshold |ρ| = 0.90; screen sample floor 250 days. R-NEFF-01: minimum common intersection 500 days; observations ≥ 10 × configurations; stability windows 80%; stability tolerance 25%. D-STAGE2-ID-01: κ threshold 10; persistence rate 30%; minimum alarms 3; materiality multiple 1.0 × fold SE; fold-SE method stationary block bootstrap, 200 resamples, mean block 10.
...
Research-round amendment rejections. (1) Fitted cross-sectional constituent hedge weights: hundreds of coefficients on under 1,000 days; hidden configuration space in penalties, inclusion rules, bounds. (2) Latent-state / Kalman-filter inventory reconstruction: no across-day persistence, so filtering degenerates to contemporaneous factor extraction, which ridge already performs implicitly at a fraction of the parameter cost; re-admits steering by unobservables that Section 4 expelled. Reconsidered only if Phase 1 shows repeated adjacent-fold sign instability among the correlated core features with approximately unchanged aggregate OOS loss — then as a constrained rank-1/rank-2 measurement model, not a dynamic filter. (3) Errors-in-variables coefficient inflation: attenuated coefficients are correct for prediction on the noisy inputs available live; dividing by reliability overweights the noisiest features; the valid residue already lives in Phase 0. (4) Per-fold SVD/PCA rotation of Stage 2: interpretation unstable across folds; converts a diagnostic into a selection device. (5) Functional (low-rank kernel) regression over the options surface: too much discretion for this sample. (6) Forecast-driven optimal execution / Almgren–Chriss staging: deferred until a robust gross edge exists at fixed entry/exit; optimising execution before signal validation converts noise into apparent timing skill. (7) Any alternative beta lookback, shrinkage, clipping, or fallback specification. (8) Any alternative N_eff estimator selected after candidate returns are observed. (9) Agreement-weighted imbalance I × Agreement: algebraically sign(I)·net²/gross, a deterministic transform of the same constituent vector R-IMB-BETA-01 uses, with an unresolved 0/0 floor on quiet days; the beta-weighted aggregation is the cleaner formulation of the same breadth economics; revisitable only as a separately registered later trial.

Amendment decision records requiring no code change: the depth-based A_t denominator is the denominator upgrade the specification already prespecifies — when depth history is sourced, the swap is a data upgrade under the existing entry, not a new trial and not a new transformation A·Q̂·(1+L_fragile). Surface velocity, auction-path surprise, strike-centroid pinning, basis–auction agreement, and surface freshness are not registered this round and are eligible later only through the standard append process, each as a replacement under the parameter budget. Section 26's expectation is unchanged.
